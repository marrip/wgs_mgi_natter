rule mark_duplicates:
  ...
  threads: 40
  shell:
    "java -jar /gatk/gatk.jar MarkDuplicatesSpark -I {input.bam} -O {output.bam} -M {output.metrics} --tmp-dir mark_duplicates --create-output-bam-index true --spark-master local[{threads}] &> {log}"
